...
export function FeatureEntryButton({ option, onClick }: FeatureEntryButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group w-full rounded-2xl border border-[#ddd8ea] bg-[#fcfbff] px-6 py-5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-[#a79be0] hover:bg-[#f6f2ff] hover:shadow-[0_10px_22px_rgba(108,91,176,0.15)]"
    >
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[24px] font-semibold leading-snug tracking-tight text-[#322d3e]">{option.title}</p>
          <p className="mt-1 text-sm leading-6 text-[#726b84]">{option.description}</p>
        </div>
