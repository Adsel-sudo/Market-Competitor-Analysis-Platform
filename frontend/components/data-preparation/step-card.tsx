  ...
  return (
    <>
      ...
      >
        ...
        {step.type === "upload" ? (
          <div className="mt-2 flex flex-col items-center gap-4 rounded-2xl bg-white/45 p-4 lg:flex-row lg:items-stretch lg:justify-between lg:gap-6">
            <div className="w-full max-w-[280px] shrink-0 text-left">
              <p className="text-xs font-medium uppercase tracking-wide text-secondary">Step {index + 1}</p>
              <h3 className="mt-1 text-lg font-semibold text-primary">{step.title}</h3>
              <p className="mt-1 text-sm text-secondary">{step.description}</p>
            </div>
            ...
            <button
              ...
              className="group relative mx-auto flex h-28 w-full max-w-[300px] shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[linear-gradient(135deg,#f3efff_0%,#ebe5ff_100%)]"
              aria-label={`${previewTitle}，点击查看大图`}
            >
              <div className="h-[78%] w-[86%] rounded-lg bg-[linear-gradient(180deg,#ffffff_0%,#f4f0ff_100%)]" />
            </button>

            <p className="w-full max-w-[220px] text-xs leading-6 text-secondary lg:pt-1">
            ...
            <button
              ...
              className="ml-auto flex min-h-[110px] w-full max-w-[320px] flex-col justify-center rounded-xl border border-dashed border-[#cdc4da] bg-white/70 px-4 text-left transition hover:bg-white"
            >
